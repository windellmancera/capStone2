...
function getUserPaymentStatus($conn, $user_id) {
    // Validate parameters
    if (!$conn || !$user_id) {
        error_log("getUserPaymentStatus: Invalid parameters - conn: " . ($conn ? 'valid' : 'null') . ", user_id: " . var_export($user_id, true));
        return null;
    }
    ...
    $sql = "SELECT u.*, mp.name as plan_name, mp.price as plan_price, mp.duration as plan_duration,
                   ...
                   (SELECT SUM(amount) FROM payment_history WHERE user_id = u.id AND payment_status = 'Pending') as pending_amount
            FROM users u 
    ...
    if (!$stmt->execute()) {
        error_log("Error executing getUserPaymentStatus statement: " . $stmt->error);
        $stmt->close();
        return null;
    }
    ...
    if (!$result) {
        error_log("Error getting result from getUserPaymentStatus statement: " . $stmt->error);
        $stmt->close();
        return null;
    }
    ...
    // Calculate balance (plan price - total paid)
    if ($user && isset($user['plan_price']) && $user['plan_price']) {
        $total_paid = $user['total_paid'] ?? 0;
        $plan_price = $user['plan_price'];
        $user['balance'] = max(0, $plan_price - $total_paid);
    } else {
        $user['balance'] = 0;
    }
    
    // Ensure all fields have default values to prevent null access warnings
    $user['plan_name'] = $user['plan_name'] ?? 'No Active Plan';
    $user['total_paid'] = $user['total_paid'] ?? 0;
    $user['completed_payments'] = $user['completed_payments'] ?? 0;
    $user['payment_status'] = $user['payment_status'] ?? 'No Payment';
    $user['membership_end_date'] = $user['membership_end_date'] ?? null;
    $user['last_payment_date'] = $user['payment_date'] ?? null;
