        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl shadow-sm p-6 mb-8">
                ...
                if (empty($recommendations)) {
                    $recommendations = array_slice($available_plans, 0, 3);
                }
                ?>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    foreach ($available_plans as $plan): 
                        $plan_data = $analytics['all_scores'][$plan['id']] ?? null;
                        $is_recommended = isset($analytics['recommendations'][$plan['id']]);
                        $recommendation_rank = $is_recommended ? array_search($plan['id'], array_keys($analytics['recommendations'])) + 1 : null;
                    ?>
                            ...
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-gray-500 text-sm mt-1">
                                        <?php 
                                        if ($plan_data && $plan_data['score'] > 80) {
                                            echo "Perfect match for you!";
                                        } elseif ($plan_data && $plan_data['score'] > 60) {
                            ...
                            <div class="mt-2">
                                <div class="flex items-center">
                                    <div class="flex-1 bg-gray-200 rounded-full h-2">
                                        <div class="bg-blue-500 h-2 rounded-full" style="width: <?php echo min(100, ($plan_data['score'] / 100) * 100); ?>%"></div>
                                    </div>
                                    <span class="ml-2 text-xs text-gray-600"><?php echo $plan_data['score']; ?>% match</span>
                                </div>
                            </div>
                            <?php endif; ?>
                        </div>
...
'Annual Plan' => ['Unlimited gym access for 365 days', 'Locker & towel service', 'Professional equipment access', 'Priority booking', 'Best value for money']
                                    ...
                                    $plan_benefits = $benefits[$plan['name']] ?? ['Full gym access', 'Professional equipment', 'Clean facilities'];
                                    foreach (array_slice($plan_benefits, 0, 4) as $benefit): 
                                    ?>
                                ...
                                <?php if ($plan_data && !empty($plan_data['reasons'])): ?>
