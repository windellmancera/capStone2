...
try {
    ...
    // Get recent equipment usage for the user
    if ($user_id) {
        $usage_sql = "SELECT eu.*, e.name as equipment_name, e.category_id,
                             ec.name as category_name, eu.start_time as usage_date,
                             TIMESTAMPDIFF(MINUTE, eu.start_time, COALESCE(eu.end_time, NOW())) as duration
        ...
        if ($usage_stmt) {
            ...
            $user['recent_equipment_usage'] = [];
            while ($row = $usage_result->fetch_assoc()) {
                $user['recent_equipment_usage'][] = $row;
            }
            $usage_stmt->close();
        }
        
        // Get user's fitness analytics
        $analytics_sql = "SELECT * FROM member_analytics WHERE user_id = ? ORDER BY date DESC LIMIT 30";
        ...
        if ($analytics_stmt) {
            ...
            $user['fitness_analytics'] = [];
            while ($row = $analytics_result->fetch_assoc()) {
                $user['fitness_analytics'][] = $row;
            }
        ...
        if (isset($user['last_visit'])) {
            ...
            $current_time = time();
            $user['days_since_last_visit'] = floor(($current_time - $last_visit) / (24 * 60 * 60));
        }
        
        // Calculate visit frequency (visits per month)
        if (isset($user['this_month_visits'])) {
            $user['visit_frequency'] = $user['this_month_visits'];
        }
        
        // Simple churn probability based on recent activity
        if (isset($user['days_since_last_visit']) && $user['days_since_last_visit'] > 30) {
            $user['churn_probability'] = 0.8;
        } elseif (isset($user['days_since_last_visit']) && $user['days_since_last_visit'] > 14) {
            $user['churn_probability'] = 0.5;
        } else {
            $user['churn_probability'] = 0.1;
        }
    }
...
<head>
    ...
    <style>
        ...
        /* Basic Notification System Styles */
        #notificationBtn {
...
<body class="bg-gray-100 homepage">
    <!-- Sidebar -->
    ...
    <div class="w-full flex justify-center items-center mt-6 mb-2">
        <header class="shadow-2xl drop-shadow-2xl px-12 py-5 flex justify-between items-center w-full max-w-7xl rounded-2xl bg-clip-padding" style="background: linear-gradient(to right, #18181b 0%, #7f1d1d 100%);">
            ...
            <div class="flex items-center space-x-10">
                ...
                <div id="notificationPopup" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
                    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto">
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <h3 id="popupTitle" class="text-lg font-semibold text-gray-900"></h3>
                                <button onclick="closeNotificationPopup()" class="text-gray-400 hover:text-gray-600 transition-colors">
                                    <i class="fas fa-times text-xl"></i>
                                </button>
                            </div>
                            <div class="mb-4">
                                <div id="popupType" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mb-2"></div>
                                <p id="popupMessage" class="text-gray-600 mb-3"></p>
                                <div class="text-sm text-gray-500">
                                    <i class="fas fa-clock mr-2"></i>
                                    <span id="popupTimestamp"></span>
                                </div>
                            </div>
                            <div class="flex justify-end space-x-3">
                                <button onclick="closeNotificationPopup()" class="px-4 py-2 text-gray-600 hover:text-gray-800 transition-colors">
                                    Close
                                </button>
                                <button id="popupMarkRead" onclick="markAsReadFromPopup()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors hidden">
                                    Mark as Read
                                </button>
                            </div>
                        </div>
                                    ...
                                    <div class="flex flex-col justify-between h-full">
                                        <div>
                                            <div class="flex items-center justify-between mb-3">
                                                ...
                                                <span class="flex-shrink-0 inline-flex items-center px-3 py-1.5 rounded-full text-sm font-bold <?php 
                                                    ...
                                                    $current_usage = isset($equipment['current_usage']) ? (int)$equipment['current_usage'] : 0;
                                                    $available = max(0, $quantity - $current_usage);
                                                    if ($available <= 0) {
                                        ...
                                        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                                            <span class="text-xs text-gray-500 font-medium">
                                                Total: <?php echo isset($equipment['quantity']) ? (int)$equipment['quantity'] : 0; ?> units
                                            </span>
                                            <span class="text-xs text-gray-500 font-medium">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Dark mode is now handled by the shared JavaScript file
        ...
        function openNotificationDropdown() {
            ...
            console.log('Dropdown classes after opening:', dropdown.classList.toString());
        }
        ...
        function closeNotificationDropdown() {
            ...
            console.log('Dropdown classes after closing:', dropdown.classList.toString());
        }
        ...
        function renderNotifications() {
            ...
            notificationList.innerHTML = notifications.map(notification => `
                <div class="notification-item p-3 border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer ${notification.read ? 'opacity-75' : ''}" 
                     data-id="${notification.id}" 
                     onclick="showNotificationPopup(${notification.id})">
                    <div class="flex items-start space-x-3">
                        <div class="flex-shrink-0 mt-1">
                            <div class="w-2 h-2 rounded-full ${getNotificationColor(notification.type)} ${notification.read ? 'opacity-50' : ''}"></div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                <p class="text-sm font-medium text-gray-900 ${notification.read ? '' : 'font-semibold'}">${notification.title}</p>
                                <span class="text-xs text-gray-500">${formatTimestamp(notification.timestamp)}</span>
                            </div>
                            <p class="text-sm text-gray-600 mt-1">${notification.message}</p>
                        </div>
                        ${!notification.read ? `
                            <button onclick="event.stopPropagation(); markAsRead(${notification.id})" class="text-xs text-blue-600 hover:text-blue-800 font-medium">
                                Mark as read
                            </button>
                        ` : ''}
                    </div>
        ...
        let currentPopupNotificationId = null;
        
        function showNotificationPopup(notificationId) {
            console.log('Showing notification popup for ID:', notificationId);
            ...
            currentPopupNotificationId = notificationId;
            ...
            document.getElementById('popupTimestamp').textContent = formatTimestamp(notification.timestamp);
            
            // Set popup type badge
            const popupType = document.getElementById('popupType');
            const typeText = notification.type.charAt(0).toUpperCase() + notification.type.slice(1);
            popupType.textContent = typeText;
            popupType.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mb-2 ${getTypeBadgeColor(notification.type)}`;
            
            // Show/hide mark as read button
            const markReadBtn = document.getElementById('popupMarkRead');
            ...
            document.getElementById('notificationPopup').classList.add('flex');
            
            // Close dropdown when popup opens
            closeNotificationDropdown();
        }
        ...
        function closeNotificationPopup() {
            ...
            document.getElementById('notificationPopup').classList.remove('flex');
            currentPopupNotificationId = null;
        }
        
        function markAsReadFromPopup() {
            if (currentPopupNotificationId) {
                markAsRead(currentPopupNotificationId);
        ...
        document.addEventListener('click', function(e) {
            ...
            if (popup && !popup.contains(e.target) && !e.target.closest('.notification-item')) {
                closeNotificationPopup();
            }
        });
