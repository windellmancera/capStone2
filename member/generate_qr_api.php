...
try {
    ...
    // Download the QR code (try fopen first)
    ...
    if ($qr_image !== false) {
        ...
        if ($qr_image_fallback !== false) {
            ...
            // Final fallback using cURL if allow_url_fopen is disabled
            $ch = curl_init($qr_url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $qr_image_curl = curl_exec($ch);
            curl_close($ch);

            if ($qr_image_curl !== false) {
                file_put_contents($qr_path, $qr_image_curl);
                $update_sql = "UPDATE users SET qr_code = ? WHERE id = ?";
                $stmt = $conn->prepare($update_sql);
                $stmt->bind_param("si", $qr_filename, $user_id);
                if ($stmt->execute()) {
                    echo json_encode([
                        ...
                        'filename' => $qr_filename,
                        'qr_url' => "../uploads/qr_codes/" . $qr_filename,
                        'message' => 'QR code generated successfully using cURL!'
                    ]);
                } else {
                    echo json_encode([
                        'success' => false, 
                ...
                echo json_encode([
                    'success' => false, 
                    'error' => 'Failed to generate QR code using all methods. Please try again later.'
                ]);
            }
        }
    }
