...
class TrainerRecommendation {
    ...
    private function checkTables() {
        ...
        $hasTrainerId = false;
        if ($hasTable) {
            $colResult = $this->conn->query("SHOW COLUMNS FROM feedback LIKE 'trainer_id'");
            $hasTrainerId = ($colResult && $colResult->num_rows > 0);
        }
        $this->has_feedback_table = ($hasTable && $hasTrainerId);
    }
