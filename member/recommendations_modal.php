...
// Get user ID from session (no redirects here since this is included after HTML output)
$user_id = $_SESSION['user_id'] ?? null;
$predictive_analysis = null;

// Only create predictive analysis if user is logged in
if ($user_id && isset($_SESSION['role']) && $_SESSION['role'] === 'member') {
    $predictive_analysis = new MemberPredictiveAnalysis($conn, $user_id);
}

// Get all recommendations (only if user is logged in)
...
function closeExerciseModal() {
    console.log('🔴 Closing exercise modal from recommendations...');
    
    // Try to close both possible modals
    const modal1 = document.getElementById('exerciseModal');
    const modal2 = document.getElementById('exerciseDetailsModal');
    ...
    setTimeout(() => {
        ...
        console.log('✅ Page fully restored and interactive from recommendations!');
    }, 100);
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        ...
        if ((modal1 && !modal1.classList.contains('hidden')) || (modal2 && !modal2.classList.contains('hidden'))) {
            console.log('🔴 ESC key pressed from recommendations, closing modal...');
            closeExerciseModal();
        }
    }
});
