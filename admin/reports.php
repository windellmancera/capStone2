...
function exportReportData() {
    ...
    $display_date_expr = "COALESCE(u.membership_start_date, u.created_at, ph.payment_date)";
...
// Build dynamic WHERE clause for filtering using the same display date rule as Payments page
// display_date = membership_start_date (if set) else created_at else payment_date
$display_date_expr_main = "COALESCE(u.membership_start_date, u.created_at, ph.payment_date)";
...
// Build recent payments query with conditional updated_at selection
$recent_payments_sql = "
    SELECT 
        ph.id,
        ph.amount,
    ...
    LEFT JOIN membership_plans mp ON u.selected_plan_id = mp.id
    WHERE $where_clause
    ORDER BY " . ($reports_has_updated_at ? "COALESCE(ph.updated_at, ph.payment_date)" : "ph.payment_date") . " DESC
    LIMIT 10
";
