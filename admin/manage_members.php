...
$expired_members = $conn->query("
    ...
    WHERE role != 'admin' 
      AND (
          (membership_end_date IS NOT NULL AND membership_end_date <= CURDATE()) OR
    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'update_status') {
            ...
            try {
                ...
                $delete_feedback = "DELETE FROM feedback WHERE user_id = ?";
                $stmt = $conn->prepare($delete_feedback);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's gym feedback records
                $delete_gym_feedback = "DELETE FROM gym_feedback WHERE user_id = ?";
                $stmt = $conn->prepare($delete_gym_feedback);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                ...
                $delete_notification_settings = "DELETE FROM notification_settings WHERE user_id = ?";
                $stmt = $conn->prepare($delete_notification_settings);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's member notes
                $delete_member_notes = "DELETE FROM member_notes WHERE user_id = ?";
                $stmt = $conn->prepare($delete_member_notes);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's member plans
                $delete_member_plans = "DELETE FROM member_plans WHERE user_id = ?";
                $stmt = $conn->prepare($delete_member_plans);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's workout plans
                $delete_workout_plans = "DELETE FROM workout_plans WHERE user_id = ?";
                $stmt = $conn->prepare($delete_workout_plans);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's exercise progress
                $delete_exercise_progress = "DELETE FROM exercise_progress WHERE user_id = ?";
                $stmt = $conn->prepare($delete_exercise_progress);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's workout_performance
                $delete_workout_performance = "DELETE FROM workout_performance WHERE user_id = ?";
                $stmt = $conn->prepare($delete_workout_performance);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's fitness data
                $delete_fitness_data = "DELETE FROM fitness_data WHERE user_id = ?";
                $stmt = $conn->prepare($delete_fitness_data);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's member analytics
                $delete_member_analytics = "DELETE FROM member_analytics WHERE user_id = ?";
                $stmt = $conn->prepare($delete_member_analytics);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's equipment views
                $delete_equipment_views = "DELETE FROM equipment_views WHERE user_id = ?";
                $stmt = $conn->prepare($delete_equipment_views);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                
                // Delete member's password reset tokens
                $delete_password_reset = "DELETE FROM password_reset_tokens WHERE user_id = ?";
                $stmt = $conn->prepare($delete_password_reset);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
                ...
                $delete_payment_history = "DELETE FROM payment_history WHERE user_id = ?";
                $stmt = $conn->prepare($delete_payment_history);
                $stmt->bind_param("i", $member_id);
                $stmt->execute();
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div class="stat-card p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-600 mb-2">Total Members</h3>
                ...
                <div class="stat-card p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-600 mb-2">Active Members</h3>
                ...
                <div class="stat-card p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-600 mb-2">Today's Check-ins</h3>
                ...
                <div class="stat-card p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-600 mb-2">Expired Memberships</h3>
                            ...
                            ?>
                                <tr class="hover:bg-gray-50 transition-all duration-200">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        // Check if member has an expired membership
                                        if ($status_expiry && $status_expiry <= $today) {
                                            $is_expired = true;
                                        }
                                        ...
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold <?php echo $statusClass; ?> border">
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 <?php echo $status === 'active' ? 'bg-green-400' : ($status === 'expired' ? 'bg-red-400' : 'bg-gray-400'); ?>"></span>
        ...
        function filterMembers() {
            ...
            memberRows.forEach((row, index) => {
                ...
                const matchesFilter = !filterValue || 
                    ...
                    (filterValue === 'none' && statusText === 'inactive');
